...
// /api/weather/only-current
router.get("/only-current", async (req, res) => {
    ...
    try {
        const response = await fetch(
            `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&appid=${process.env.API_KEY}&units=metric&exclude=minutely,hourly,daily,alerts`
        ).then((res) => res.json());
